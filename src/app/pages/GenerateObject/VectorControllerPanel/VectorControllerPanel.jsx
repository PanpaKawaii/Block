...
export default function VectorControllerPanel({
        ...
        setVectors((prev) =>
            prev.map((vector) =>
                vector.id === vectorId
                    ? {
                        ...vector,
                        name: `${index == 1 ? name + '-' + vector.name?.split('-')?.[1]?.split(' ')?.[0] : vector.name?.split('-')?.[0] + '-' + name} (${xB - xA},${yB - yA},${zB - zA})`,
                        xCoordinateA: xA,
                        yCoordinateA: yA,
                        zCoordinateA: zA,
                        xCoordinateB: xB,
    ...
    return (
        <ControlPanel
            titleName={'Vector Controller'}
            extraClassName={'vector-controller-panel-container'}
            collapseMenu={toggleMenu}
            size={toggleStepFunction == 'vector' ? (selectedFace ? 'size_1_2' : 'size_1_1') : (selectedFace ? 'size_1_4' : 'size_1_3')}
            collapseController={collapseController}
            entity={vectors}
            setEntity={setVectors}
            addEntity={addVector}
                ...
                {vectors.map((vector) => (
                    <div key={vector.id} className={`card ${vector.visible == 0 ? 'invisible' : ''} ${vector.id == selectedVectorId ? 'dash-box' : ''}`}>
                        <div className='header'>
                            ...
                            />
                            <div className={`name-group ${(openedVectorId.includes(vector.id) || !toggleMenu) ? 'expanse' : ''}`}>
                                <MovingLabelInput
                                    type={'text'}
                                    value={vector?.name || ''}
